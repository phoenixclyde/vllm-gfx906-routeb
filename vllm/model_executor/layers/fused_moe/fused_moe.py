...
def fused_moe_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        if USE_TD:
            ...
            if on_gfx906:
                # gfx906 triton needs an int fill value so the masked lanes stay
                # type-consistent with the loaded tile.
                b = tl.load(
        ...
        if use_int8_w8a16:
            ...
            if group_k > 0 and group_n > 0:
                ...
                if SWAP_AB:
                    ...
                    # a_scale is not loaded on gfx906 (see the branch above).
...
def get_default_config(
    ...
    if dtype == "fp8_w8a8" and block_shape is not None:
        # gfx906 (MI50): tuned tile table from benchmarks.
        if on_gfx906():
            if M <= 1:
