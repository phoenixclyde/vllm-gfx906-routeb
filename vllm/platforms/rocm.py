...
# gfx906 是 GCN5/Vega，不属于 CDNA。上游用 "gfx9" 前缀判定会把 gfx906 误判为 CDNA，
# 必须显式排除，否则所有 _ON_CDNA 分支都会走错路径（静默错误）。
_ON_CDNA = (
    any(arch in _GCN_ARCH for arch in ["gfx9", "gfx1250"]) and not _ON_GFX906
)
...
class RocmPlatform(Platform):
    ...
    def supports_fp8(cls) -> bool:
        return on_cdna() or on_gfx12x() or on_gfx906()
